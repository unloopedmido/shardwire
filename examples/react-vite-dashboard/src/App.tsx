...
import { ShardwireProvider, useShardwire, useShardwireAction, useShardwireListener } from '@shardwire/react';
	...
	return (
		<div className="dashboard-stack">
			...
			<section className="dashboard-section" aria-labelledby="action-demo-heading">
				...
				<p className="dashboard-muted">
					Set <code>VITE_DEMO_CHANNEL_ID</code> in <code>.env</code> to enable the button (see <code>.env.example</code>
					).
				</p>
				<button
					type="button"
					...
					onClick={() => {
						...
						void sendMessage.invoke({ channelId: demoChannelId, content: 'Hello from @shardwire/react' });
					}}
				>
					{sendMessage.isPending ? 'Sending…' : 'Send test message'}
				</button>
				{sendMessage.lastResult ? (
					<pre className="dashboard-pre" aria-label="Last action result JSON" style={{ whiteSpace: 'pre-wrap' }}>
