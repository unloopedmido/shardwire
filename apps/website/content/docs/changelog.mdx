---
title: Changelog
description: Notable Shardwire releases from v1.0.0 onward — API additions, behavior changes, and operational updates.
---

This page lists **notable changes in the `shardwire` package** from **v1.0.0** through the current release. Earlier pre–1.0 history lived in the legacy generic bridge API and is intentionally omitted here.

Release tags on GitHub: [`v1.0.0` … current](https://github.com/unloopedmido/shardwire/tags).
...
- Documentation: [incremental adoption](/docs/guides/incremental-adoption), golden [recipes](/docs/guides/recipe-moderation-worker) (moderation, interactions, analytics, music controller), [CI contract validation](/docs/operations/ci-contract-validation), [secret cookbook](/docs/operations/secret-cookbook), [remote bridge (wss)](/docs/operations/remote-bridge), [observability](/docs/operations/observability), [slash command registration](/docs/guides/slash-command-sync), [product boundaries](/docs/concepts/product-boundaries), and [custom domain contracts (spike)](/docs/concepts/custom-domain-contracts).
- Added optional workspace package **`@shardwire/react`** with hooks around **`connectBotBridge`** (`useShardwireBridge`, `useShardwireCapabilities`, `useShardwireEvent`).
- Root **`verify`** / **`build`** scripts now typecheck and build **`@shardwire/react`** after **`shardwire`**.
