...
- **Node.js 18–21** (no global `WebSocket`): polyfill `globalThis.WebSocket` from **`ws`** before connecting, or run the app process on **Node 22+** — see [Troubleshooting](https://shardwire.js.org/docs/operations/troubleshooting/#nodejs-below-22-connectbotbridge-requires-globalthiswebsocket).
...
This tab covers the **`@shardwire/react`** npm package. It versions **independently** from **`shardwire`**; see **`dependencies`** / **`peerDependencies`** in `package.json` on each release for the **`shardwire`** range and **`react`** peer.

npm: [`@shardwire/react`](https://www.npmjs.com/package/@shardwire/react)

## 0.1.3

- **`shardwire`** is now a **runtime `dependency`** (`^1.9.2`) so `npm install @shardwire/react react` is enough for hook-only apps. **`react`** remains the only **peer dependency**.
