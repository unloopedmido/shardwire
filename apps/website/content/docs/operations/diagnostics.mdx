...
- `formatShardwireDiagnosis()` for **human-readable** reports (CI logs, deploy logs, strict-startup output)
...
if (!report.ok) {
	console.error(formatShardwireDiagnosis(report, { title: 'preflight' }));
}
```
