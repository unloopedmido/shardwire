...
- Added bridge modes on `createBotBridge`: **`split`** (default), **`hybrid`** (split + optional `bot.client()` exposure), and **`single-process`** (no WebSocket transport; app bridge via `bot.app()`).
- Added optional raw passthrough action support: bot-side `raw.enabled` + `raw.allow`/`raw.deny`, app-side `app.actions.runRaw(...)` and `app.raw(...)`.
- Added single-process in-memory app bridge implementation with strict-ready/preflight support and parity with the app bridge API shape.
- Added docs for migration paths, hybrid mode, single-process mode, and Discord.js parity mapping.

## 2.1.1

- Dependency refresh release from PR 24: bumped package dependencies/devDependencies to newer compatible versions with no public API changes.

## 2.1.0

- **Voice state reads:** added **`fetchVoiceState({ guildId, userId })`** as a built-in action that returns the normalized **`BridgeVoiceState`** snapshot for the target guild member.
- Regenerated action-model reference docs for the new **`FetchVoiceStateActionPayload`** and the updated action payload/result maps.
- Updated the Shardwire skill guidance so voice-state read/write coverage now includes **`fetchVoiceState`** alongside the existing voice event and mutation actions.
