...
Optional **`@shardwire/react`** provides hooks around `connectBotBridge` so dashboard UIs do not need to wire lifecycle by hand. See the README in the repository (`packages/react`) and the [music controller recipe](/docs/guides/recipe-music-controller). Core `shardwire` stays free of a React peer dependency.
