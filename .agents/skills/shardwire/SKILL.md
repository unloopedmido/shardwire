...
- **`@shardwire/react`** (workspace package) — optional hooks around **`connectBotBridge`**: `useShardwireBridge`, `useShardwireCapabilities`, `useShardwireEvent`. Hooks resolve **`shardwire/client`** (not the root export) so Vite/browser bundles avoid **`discord.js`** / **`zlib-sync`**. Source and examples: `packages/react/README.md`; does not bundle React — install `react`, **`shardwire` ≥ 1.9.1**, and `@shardwire/react` together.
