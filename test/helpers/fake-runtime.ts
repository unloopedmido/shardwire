...
type ActionHandlerMap = {
  [K in BotActionName]?: (payload: BotActionPayloadMap[K]) => Promise<BotActionResultDataMap[K]> | BotActionResultDataMap[K];
};
...
export class FakeDiscordRuntime implements DiscordRuntimeAdapter {
  ...
  on<K extends BotEventName>(name: K, handler: (payload: BotEventPayloadMap[K]) => void): Unsubscribe {
    ...
    if (existing) {
      ...
      this.eventHandlers[name] = new Set([(handler as never) as (payload: BotEventPayloadMap[BotEventName]) => void]) as EventHandlers[K];
    }
    return () => {
