...
`BridgeCapabilityError` is thrown from `app.ready()` when a handler is registered for an event the secret + intents combo does not allow. Remove the handler or widen scope (carefully).
